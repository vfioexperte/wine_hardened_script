...
parser.add_argument('-version', action='store_true', default=False, dest='boolean_version', help='output onlay app version then exit');
parser.add_argument('-Steam', action='store_true', default=False, dest='boolean_Steam', help="enabled Steam mode\n" + "use this mode only with protontricks command\n" + "you will find the APPID with protontricks -s \"game name\"\n" + "protontricks  -c \"/usr/bin/wine-security-gui -Steam\" APPID");
parser.add_argument('-Steam_auto_remove_protect', action='store_true', default=False, dest='boolean_Steam_auto_protct', help="Steam auto protect");
parser.add_argument('-Steam_auto_remove_protect', action='store_true', default=False, dest='boolean_Steam_auto_remove_prtect', help="Steam remove auto protect");
parser.add_argument('-Steam_all_auto_protect', action='store_true', default=False, dest='boolean_Steam_all_auto_prtect', help="Steam all auto protect");
parser.add_argument('-Steam_all_auto_remove_protect', action='store_true', default=False, dest='boolean_Steam_all_auto_remove_prtect', help="Steam remove auto protect");
parser.add_argument('--auto', action='store_true', default=False, dest='boolean_auto', help='auto downlaod and update proton modus enable')
parser.add_argument('-debug', action='store_true', default=False, dest='boolean_debug', help='entickler option')
results = parser.parse_args();
if (results.boolean_nogui == True):
    gui = 0
if (results.boolean_auto == True):
    auto = 1;
...
if(results.boolean_version == True):
    exit();
if(results.boolean_Steam == True):
    steammode = 1;
if(results.boolean_Steam_auto_protct == True):
    steamauto = 1;
    steammode = 1;
if(results.boolean_Steam_auto_remove_prtect == True):
    steamauto = 2;
    steammode = 1;
if(results.boolean_Steam_all_auto_prtect == True):
    steamall = 1;
    steamauto = 1;
if(results.boolean_Steam_all_auto_remove_prtect == True):
    steamall = 1;
    steamauto = 2;
    ...
    def remove_wine_links(self):
        ...
        if(os.path.isdir(wine_drive_c_user) == True):
            ...
            if(anser == QtWidgets.QMessageBox.Yes):
                ...
                print("copy");
        self.change_WINEPREFIX(self.WINEPREFIX);
    def read_userdir_config_file():
        if(os.path.isfile(home + "/.config/user-dirs.dirs") == False):
            print("Errror userdir config file not found");
            return -1;
        file1 = open(home + "/.config/user-dirs.dirs", "r");
        file1.seek(0, 2);
        size = file1.tell();
        file1.seek(0, 0);
        s1 = file1.read(size);
        ...
        while True:
            ...
            if(len(s3) >= 1):
                ...
                s4 = s3.split("=");
                if(len(s4) >= 2):
                    if(s4[0] == "XDG_DESKTOP_DIR"):
                        dirs.append(s4[1]);
                    elif(s4[0] == "XDG_DOWNLOAD_DIR"):
                        dirs.append(s4[1]);
                    elif(s4[0] == "XDG_TEMPLATES_DIR"):
                        dirs.append(s4[1]);
                    elif(s4[0] == "XDG_PUBLICSHARE_DIR"):
                        dirs.append(s4[1]);
                    elif(s4[0] == "XDG_DOCUMENTS_DIR"):
                        dirs.append(s4[1]);
                    elif(s4[0] == "XDG_MUSIC_DIR"):
                        dirs.append(s4[1]);
                    elif(s4[0] == "XDG_PICTURES_DIR"):
                        dirs.append(s4[1]);
                    elif(s4[0] == "XDG_VIDEOS_DIR"):
                         dirs.append(s4[1]);
                    else:
                        print("errror");
            i = i +1;
